...
from math_gymnasium.dynamical_systems.linear_system import (
    rollout_linear_spiral_partial_derivative,
)


@pytest.fixture
def setup_linear_spiral_sys_spaces() -> Tuple[
    np.ndarray,
    np.ndarray,
...
class TestRolloutLinear_spiralPartialDerivative:
    def test_default(self, setup_linear_spiral_sys_spaces):
        (t_time_space, t_dt_space, t_init_coord, t_coord, t_derivative) = (
            setup_linear_spiral_sys_spaces
        )
        t_time_space_freeze = t_time_space.copy()
        coords = rollout_linear_spiral_partial_derivative(
            t_time_space, initiale_coordinates=t_init_coord,
        )
        assert np.array_equal(
            t_time_space_freeze, t_time_space
        ), "original array was overiden"
        assert coords.shape == (t_time_space.shape[0], 3)

        # (Nice to have) ToDo: implement stability tests via coordinate rollout (ref task MG34)
        # assert coords == pytest.approx(t_coord)

    def test_output_coord_using_delta_time(self, setup_linear_spiral_sys_spaces):
        (t_time_space, t_dt_space, t_init_coord, t_coord, t_derivative) = (
            setup_linear_spiral_sys_spaces
        )
        t_dt_space_freeze = t_dt_space.copy()
        coord = rollout_linear_spiral_partial_derivative(
            t_dt_space, initiale_coordinates=t_init_coord, time_space_is_delta_time=True
        )
        assert np.array_equal(
            t_dt_space_freeze, t_dt_space
        ), "original array was overiden"
